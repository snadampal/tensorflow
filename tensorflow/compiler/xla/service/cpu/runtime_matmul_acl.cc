...
//TODO: make this ENABLE_ACL a bazel config
#if 1
//#ifdef ENABLE_ACL
...
void MatMulF32(const void* run_options_ptr, float* out, float* lhs, float* rhs,
	...
	if (acl_conf.is_trans_lhs) {
		acl_conf.lhs_acc_info
			= arm_compute::TensorInfo(arm_compute::TensorShape(k, m, batch),
				1, arm_compute::DataType::F32);
	}
	if (acl_conf.is_trans_rhs) {
                acl_conf.rhs_acc_info = arm_compute::TensorInfo(
                        arm_compute::TensorShape(n, k, 1, batch), 1,
                        arm_compute::DataType::F32);
        }

	acl_conf.lhs_info
            = arm_compute::TensorInfo(arm_compute::TensorShape(m, k, batch), 1,
                    arm_compute::DataType::F32);
	acl_conf.rhs_info
            = arm_compute::TensorInfo(arm_compute::TensorShape(k, n, 1, batch),
                    1, arm_compute::DataType::F32);
	acl_conf.out_info
            = arm_compute::TensorInfo(arm_compute::TensorShape(m, n, 1, batch),
                    1, arm_compute::DataType::F32);
	...
	if (acl_conf.is_trans_lhs) {
                ...
                if (acl_trans_lhs_st.error_code() != arm_compute::ErrorCode::OK) {
                        std::cout << "lhs transpose validation failed" << std::endl;
                }
        }
	if (acl_conf.is_trans_rhs) {
		...
		if (acl_trans_rhs_st.error_code() != arm_compute::ErrorCode::OK) {
			std::cout << "rhs transpose validation failed" << std::endl;
		}
	}

	// Validate ACL GEMM
	auto acl_st = arm_compute::NEGEMM::validate(&acl_conf.rhs_info, &acl_conf.lhs_info,
		nullptr, &acl_conf.out_info, acl_conf.alpha, 0.0f, acl_conf.gemm_info);
	if (acl_st.error_code() != arm_compute::ErrorCode::OK) {
		std::cout << "validate acl GEMM FAILED" << std::endl;
	}
	...
	//configure the acl obj with the config
        ...
        if (acl_conf.is_trans_lhs) {
            ...
            acl_obj.trans_lhs.configure(
                    &acl_obj.lhs_acc_tensor, &acl_obj.lhs_tensor);
        }
        if (acl_conf.is_trans_rhs) {
            acl_obj.rhs_acc_tensor.allocator()->init(acl_conf.rhs_acc_info);
            acl_obj.trans_rhs.configure(
                    &acl_obj.rhs_acc_tensor, &acl_obj.rhs_tensor);
        }
	// Configure GEMM
	acl_obj.gemm.configure(&acl_obj.rhs_tensor, &acl_obj.lhs_tensor,
                nullptr, &acl_obj.out_tensor, acl_conf.alpha, 0.0f, acl_conf.gemm_info);
